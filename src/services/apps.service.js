...
async function getAllAppTypes(filters = {}) {
	...
	if (filters.type) {
		...
		if (!['SERVER', 'APP'].includes(t)) {
			throw createError(
				400,
				`Invalid type filter '${filters.type}'. Must be SERVER or APP.`
			);
		}
		where.type = t;
	}
...
async function getAppAtSite(rawAppKey, rawSiteCode) {
	...
	if (!siteIp) {
		throw createError(
			404,
			`IP untuk app '${appKey}' di site '${siteCode}' tidak ditemukan.`
		);
	}
...
async function quickLookup(rawAppKey, rawSiteCode) {
	if (!rawAppKey || !rawSiteCode) {
		throw createError(
			400,
			'Both "app" and "site" query parameters are required.'
		);
	}
	...
	if (!siteIp) {
		throw createError(
			404,
			`No IP found for app '${appKey}' at site '${siteCode}'.`
		);
	}
